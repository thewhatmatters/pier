...
| Cursor | Running or off, plus the open project. Subtitle lists working agents by the first prompt in their transcript under `~/.cursor/projects/*/agent-transcripts`. The tile refreshes on the dock pulse; transcripts are rescanned at most every ~8s. A session is working until Cursor writes `turn_ended`, not for two minutes after. Cloud sessions (`bc-` ids) are included when they write locally |
